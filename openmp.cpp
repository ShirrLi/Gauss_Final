    ...
    for(i=0;i<elinenum;i++){//遍历所有被消元行
        while(eline[i].null()==0){//只要被消元行非空就一直异或消元
            ...
            if(eliminer[s].null()==0)//若存在对应消元子
                eline[i].gauss_xor(eliminer[s]);//进行异或消元
            else{
...
void gauss_omp(){
    ...
    for(i=col-1;i>=0;i--)//遍历消元子所有可能行号
        if(eliminer[i].null()==0){//若该行下存在消元子
			...
			//若该行号下消元子为空
            #pragma omp single
            for(j=0;j<elinenum;j++){//找到匹配的被消元行
                if(eline[j].start==i){
            ...
            #pragma omp for schedule(static,1)
            for(j=tmp;j<elinenum;j++){//升格之后若找到了匹配的被消元行，则进行消元，否则tmp的值会不满足循环条件，这一轮消去结束
                if(eline[j].start==i)
